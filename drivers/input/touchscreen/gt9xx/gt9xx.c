...
#ifdef CONFIG_TOUCHSCREEN_HIDEEP_IST771X
extern int tp_connected;
#endif

#ifdef CONFIG_LENOVO_SMART_PAD_SUPPORT
extern int notify_touch_event(int finger, int x, int y, int w);
#endif
...
void gtp_int_sync(struct goodix_ts_data *ts_data, s32 ms);
...
static void gtp_touch_down(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
    ...
    if ( ts->tp_mode != 3)
     {
	if ( notify_touch_event(id, x,y,w))
	return;
     }
#endif
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    u8  point_data[2 + 1 + 9 * GTP_MAX_TOUCH + 1]={GTP_READ_COOR_ADDR >> 8, GTP_READ_COOR_ADDR & 0xFF};
    u8  touch_num = 0;
    u8  finger = 0;
    ...
    if (touch_num > 1)
    {
        u8 buf[9 * GTP_MAX_TOUCH] = {(GTP_READ_COOR_ADDR + 10) >> 8, (GTP_READ_COOR_ADDR + 10) & 0xff};

        ret = gtp_i2c_read(ts->client, buf, 2 + 9 * (touch_num - 1));
        memcpy(&point_data[12], &buf[2], 9 * (touch_num - 1));
    }

#if (GTP_HAVE_TOUCH_KEY || GTP_PEN_HAVE_BUTTON)
    key_value = point_data[3 + 9 * touch_num];
    ...
    if (pre_touch || touch_num)
    {
        ...
        for (i = 0; i < GTP_MAX_TOUCH; i++)
        {
            ...
            if ((touch_index & (0x01<<i)))
            {
                ...
                input_y  = coor_data[pos + 5] | (coor_data[pos + 6] << 8);
                input_w  = coor_data[pos + 7] | (coor_data[pos + 8] << 8);
    ...
    if (touch_num)
    {
        ...
        {
            coor_data = &point_data[i * 9 + 3];

            id = coor_data[1] & 0x0F;
            input_x  = coor_data[3] | (coor_data[4] << 8);
            input_y  = coor_data[5] | (coor_data[6] << 8);
            input_w  = coor_data[7] | (coor_data[8] << 8);
...
//gpiod_direction_output(ts_data->gpiod_inter, 0);
    msleep(ms);
    GTP_GPIO_AS_INT(gtp_int_gpio);
//gpiod_direction_input(ts_data->gpiod_inter);
    irq_set_irq_type(ts_data->client->irq, irq_table[ts_data->int_trigger_type]);
}
...
//	gpiod_direction_output(ts->gpiod_reset, 0);
    msleep(ms);                         // T2: > 10ms
    // HIGH: 0x28/0x29, LOW: 0xBA/0xBB
    GTP_GPIO_OUTPUT(gtp_int_gpio, client->addr == 0x14);
//	gpiod_direction_output(ts->gpiod_inter, client->addr == 0x14);
    msleep(2);                          // T3: > 100us
    GTP_GPIO_OUTPUT(gtp_rst_gpio, 1);
//	gpiod_direction_output(ts->gpiod_reset, 1);
    msleep(6);                          // T4: > 5ms

    GTP_GPIO_AS_INPUT(gtp_rst_gpio);    // end select I2C slave addr
//	gpiod_direction_input(ts->gpiod_reset);
...
static u8 gtp_get_minvalue(struct goodix_ts_data *ts)
{
	s8 ret = 1;
	s8 retry = 0;
	u8 i2c_control_buf[3] = {(u8)0x00, (u8)0x00, 0x00};

	GTP_DEBUG_FUNC();
	while(retry++ < 5 )
	{
		i2c_control_buf[0] = 0x80;
		i2c_control_buf[1] = 0xA8;
		ret = gtp_i2c_read(ts->client, i2c_control_buf, 3);
		if (ret < 0 )
		{
			GTP_ERROR("failed to get minvalue in 0x80A8, %d", retry);
			msleep(10);
			continue;
		}
		GTP_INFO("minvalue %d\n", i2c_control_buf[2]);
	...
	return i2c_control_buf[2];

}
static s8 gtp_get_mode(struct goodix_ts_data *ts)
{
    s8 ret = -1;
    u8 status_buf[3] = {0x81, 0x4D, 0x00};


    ret = gtp_i2c_read(ts->client, status_buf, 3);
    if (ret >= 0) {
	GTP_DEBUG("%s %x\n", __func__, status_buf[2]);
    }else {
	GTP_DEBUG("%s error\n", __func__);
    }
    return status_buf[2];
}
static s8 gtp_set_mode(struct goodix_ts_data *ts)
{
    s8 ret = -1;
    s8 retry = 0;
    u8 i2c_control_buf[3] = {(u8)0x0, (u8)0x0, 0x10};


    i2c_control_buf[2] = 0x10 |ts->tp_mode;
    GTP_DEBUG("Entering set tp mode 0x%x.", i2c_control_buf[2]);
    while(retry++ < 5)
    {
        i2c_control_buf[0] = 0x80;
        ...
        if (ret < 0)
        {
            GTP_DEBUG("failed to set tp mode into 0x8046, %d", retry);
            continue;
        }
        i2c_control_buf[0] = 0x80;
        ...
        if (ret > 0)
        {
            GTP_INFO("tp mode %d.", ts->tp_mode);
            return ret;
        }
        msleep(10);
    }
    GTP_ERROR("GTP send tp mode cmd failed.");
    return ret;
...
static s8 gtp_enter_sleep(struct goodix_ts_data * ts)
{
    ...
    GTP_GPIO_OUTPUT(gtp_int_gpio, 0);
//	gpiod_direction_output(ts->gpiod_inter, 0);
    msleep(5);
...
static s8 gtp_request_irq(struct goodix_ts_data *ts)
{
    ...
    if (ret)
    {
        ...
        GTP_GPIO_FREE(gtp_int_gpio);
//	gpiod_direction_input(ts->gpiod_inter);
...
static int gtp_power_switch(struct i2c_client *client, int on)
{
	...
	static struct regulator *vdd2 = NULL;
	static struct regulator *vcc_i2c = NULL;
	int ret, rc;

	if (!vdd_ana) {
		vdd_ana = regulator_get(&client->dev, "VPROG6A");
		if (IS_ERR(vdd_ana)) {
	...
	if (regulator_count_voltages(vdd_ana) > 0) {
		ret = regulator_set_voltage(vdd_ana, 3300000,
					   3300000);
		if (ret) {
			GTP_ERROR("Regulator set_vtg VPROG6A failed vdd rc=%d\n", ret);
			goto ERR_GET_VCC;
		}
	}

	if (yeti_hw_ver >= 2 )
		vdd2 = regulator_get(&client->dev, "VPROG3B");
	else
		vdd2 = regulator_get(&client->dev, "VPROG4D");
	if (IS_ERR(vdd2)) {
		rc = PTR_ERR(vdd2);
		GTP_ERROR( "Regulator vdd get failed vdd rc=%d\n", rc);
		goto ERR_GET_VCC;
	}

	if (regulator_count_voltages(vdd2) > 0) {
		rc = regulator_set_voltage(vdd2, 3300000,
					   3300000);
		if (rc) {
			GTP_ERROR("Regulator set_vtg failed vdd rc=%d\n", rc);
			goto ERR_GET_VCC;
		}
	}

	if (!vcc_i2c) {
		vcc_i2c = regulator_get(&client->dev, "VPROG5B");
		if (IS_ERR(vcc_i2c)) {
	...
	if (regulator_count_voltages(vcc_i2c) > 0) {
		rc = regulator_set_voltage(vcc_i2c, 1800000,
					   1800000);
		if (rc) {
			GTP_ERROR( "Regulator set_vtg failed vcc_i2c rc=%d\n", rc);
			goto ERR_GET_VCC;
		}
	}
...
static void gt9xx_sleep(struct goodix_ts_data *ts)
{
	...
	ret = gtp_i2c_write(ts->client, i2c_control_buf, 3);
	//ret = gtp_i2c_write(ts->client, GTP_REG_SLEEP, 5);
	if (ret <= 0 ) {
		GTP_ERROR("sleep cmd failed");
...
static void gt9xx_wakeup(struct goodix_ts_data *ts)
{
	...
	ret = gtp_i2c_test(ts->client);
	if (ret <= 0) {
		GTP_ERROR("wake up failed");
		return;
	}

	gtp_int_sync(ts, 50);
	gtp_irq_enable(ts);
	GTP_INFO("wakeup from sleep");
...
static ssize_t tp_mode_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (( n>=0) && ( n<4)) {
    	ts_drv->tp_mode = simple_strtoul(buf, NULL, 0);
    	if ( !ts_drv->gtp_is_suspend ) {
    		gtp_get_mode(ts_drv);
    		gtp_set_mode(ts_drv);
	}else{
		GTP_INFO("suspended yet\n");
	}
    }
    return count;
}
...
static DEVICE_ATTR(tp_mode, S_IRWXUGO, tp_mode_show, tp_mode_write);
 ...
 * attribute group
 *******************************************************************************/
static struct attribute *goodix_ts_sysfs_entries[] =
{
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	gtp_rst_gpio = GTP_RST_PORT;//desc_to_gpio(devm_gpiod_get_index(&client->dev, "gt9xx_gpio_rst", 0));
	gtp_int_gpio =  GTP_INT_PORT;//desc_to_gpio(devm_gpiod_get_index(&client->dev, "gt9xx_gpio_int", 1));
	//gpio_rst =devm_gpiod_get_index(&client->dev, "gt9xx_gpio_rst", 0);
	//gpio_int =devm_gpiod_get_index(&client->dev, "gt9xx_gpio_int", 1);
#endif
...
/*
                ret = gpiod_direction_input(gpio_int_gpio);
        if (ret) {
               printk("www gpio %d dir set failed\n", desc_to_gpio(gpio_int));
                return ret;
        }

        ret = gpiod_to_irq(gpio_int_gpio);
        if (ret < 0) {
                printk("www gpio %d irq failed\n", desc_to_gpio(gpio_int));
                return ret;
        }

        if (client->irq < 0)
                client->irq = ret;
        ts->gpiod_reset = gpio_rst;
        ts->gpiod_inter = gpio_int;


        gpiod_direction_input(gpio_rst);
        gtp_reset_guitar(ts->client, 20);

        printk("www  reset_gpi_no: %d, int_gpio_no: %d, irq: %d, client_irq: %d\n",
                        desc_to_gpio(gpio_rst), desc_to_gpio(gpio_int), ret, client->irq);
...
*/
    ...
    ret =  sysfs_create_group(&client->dev.kobj, &goodix_ts_attr_group);

     if(ret){
           GTP_ERROR("CANNOT create control file nood");
     }
	...
	if (ret < 0) {
		GTP_ERROR( "unable to create suspend entry");
		goto out;
	}
...
out:
    ...
    printk("##C Face: KB driver probe success##\n");
