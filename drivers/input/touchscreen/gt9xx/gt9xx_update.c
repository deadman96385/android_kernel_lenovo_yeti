...
struct goodix_ts_data *ts_data = i2c_get_clientdata(i2c_connect_client);
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
    ...
    printk("#GT9xx FILE PID=%s,VID=%04x; IC PID=%s,VID=%04x#\n",fw_head->pid,fw_head->vid,update_msg.ic_fw_msg.pid,update_msg.ic_fw_msg.vid);
...
s32 gup_update_proc(void *dir)
{
    ...
    if(FAIL == ret)
    {
        GTP_INFO("[update_proc]Check *.bin file fail.");
        goto file_fail;
    }
