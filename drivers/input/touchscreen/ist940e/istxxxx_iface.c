...
istcore_iface_llseek(struct file *file, loff_t off, int whence)
{
    ...
    ISTCORE_INFO("%s off = 0x%08x, whence = %d",__FUNCTION__,(unsigned int)off,whence);
    if(file->private_data == NULL)
        return -EFAULT;
    ...
    switch(whence)
    {
        ...
        /* SEEK_CUR */
        case 1  :
              ISTCORE_INFO("%s set mode off = 0x%08x",__FUNCTION__,(unsigned int)off);
              if(off == 0x1000)
              {
                      drv_info->im_r_en = 1;
        ...
        case 2  :
            ...
            ISTCORE_INFO("%s ISTCORE_IFACE_CFG : %d", __FUNCTION__,drv_info->im_size);
            ISTCORE_INFO("%s ISTCORE_IFACE_CFG : %d", __FUNCTION__,drv_info->vr_size);
            newpos = file->f_pos;
            break;
        default:
            return -EINVAL;
