...
pgm_w_reg(struct i2c_client *client, u32 addr, u32 data)
{
    ...
    if (ret < 0)
    {
    	ISTCORE_ERR("i2c_master_send fail\n");
        goto err;
    }
...
pgm_r_mem(struct i2c_client *client, u32 addr, struct pgm_packet *packet, u32 len)
{
    ...
    if (ret < 0)
    {
    	ISTCORE_ERR("i2c_master_recv fail\n");
        goto err;
    }
...
int is_update_necessary(struct ist510e *ts, unsigned char *pres)
{
    ...
    ISTCORE_INFO("binary factory        : %04x\n", bin_dwz_info.factory_id);
/*Modify begin by lenovo xucm1,YETIM-84,change update condition from < to !=  */
    ...
    ver_cmp |= ((ts->dwz_info.factory_id != bin_dwz_info.factory_id) << 4);
/*Modify end 20160429 */
    if (ver_cmp != 0)
    {
...
ist_load_dwz(struct ist510e *ts)
{
    ...
    printk("IST940 firmware version:boot=%x,core=%x,custom=%x,vr=%x,factory id=%x \n",ts->dwz_info.ver_b,ts->dwz_info.ver_c,ts->dwz_info.ver_d,ts->dwz_info.ver_v,ts->dwz_info.factory_id);
