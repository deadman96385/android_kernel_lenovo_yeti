...
#if defined(CONFIG_CHRLED_GPLED)
	...
	if (val) {
		...
		if((bq->main_charger_flag == 1)&&(sch_flag==1))
		{
			...
			if ((bq->usb.type==6) || (bq->usb.type==12)) {
				boost_flag=0;
				printk(KERN_ERR "bq2589x_enable_charging =start==chrg_boost_wrkr\n");
				printk("cancel sch_flag=%d\n",sch_flag);
...
#endif
	printk(KERN_ERR "%s:%d",__func__,val);
	return ret;
}
...
static inline int bq2589x_enable_charger(struct bq2589x *bq, bool val)
{
	...
	if((bq->main_charger_flag == 1)&&(sch_flag==0)&&(val==1))
	{
		sch_flag=1;
		if((bq->usb.type==6) || (bq->usb.type==12)) {
			printk(KERN_ERR "bq2589x_enable_charger =start==chrg_boost_wrkr\n");
			...
			printk(KERN_ERR"bq2589x_enable_charger =start==aca_work\n");
...
void config_bq25892(void)
{
	...
	if(!g_bq->charge_en){
			printk(KERN_ERR "exit config bq25892 begin");
		goto  exit_config;
	}
...
//					bq2589x_disable_chr(g_bq);
		...
		printk("unlock====full and release\n");
	}
...
exit_config:
	printk(KERN_ERR "exit config bq25892 end");
}
...
#define USBSRCDETSTAT 0x5E29
#define SPWRSRC			  0x6e1e
...
static int read_pmic_reg(struct bq2589x *bq,u16 addr,u8 *val)
{
	...
	if (ret == -EIO) {
		dev_err(bq->dev, "%s:I/O Error(%d): addr:0x%.4x\n",
				__func__, ret,addr);
		return ret;
	}
	*val = ret;
...
static int pmic_get_charger_type(struct bq2589x *bq)
{
	int ret, i = 0,h=0;
	int time = 0;
	u8 val;
	...
	msleep(100);
	do {
		ret = read_pmic_reg(bq,USBSRCDETSTAT, &val);
		if (ret)
			return 0;
		i++;
		dev_dbg(bq->dev, "aaronZ... Read USBSRCDETSTATUS val: %x\n", val);
	...
	if ((val & USBSRCDET_SUSBHWDET_DETSUCC) !=
			USBSRCDET_SUSBHWDET_DETSUCC) {
		dev_err(bq->dev, "aaronZ... Charger detection unsuccessful after %dms\n",
			i * USBSRCDET_SLEEP_TIME);
		return 0;
	}

	chgr_type = (val & USBSRCDET_USBSRCRSLT_MASK) >> 2;
	dev_dbg(bq->dev, "aaronZ...Charger type after detection complete: %d\n",
...
/*	if(power_off_on)
	...
	chg_type = pmic_get_charger_type(bq);

	dev_info(bq->dev, "irq_workfunc, REG0B=0x%x, REG0C=0x%x,chg_type=%d,\n", status, fault,chg_type);
	dump_mainchg_reg(bq);
	/*
	...
	if (temp == BQ2589X_CHRG_STAT_CHGDONE) {
		...
		if((pg_stat == BQ2589X_PG_GOOD) &&
		   (chg_type == POWER_SUPPLY_CHARGER_TYPE_USB_DCP)){
			cancel_delayed_work_sync(&bq->chrg_boost_wrkr);
			queue_delayed_work(system_freezable_wq, &bq->chrg_boost_wrkr,  0 );
			bq2589x_enable_charger(bq, true);
		}
