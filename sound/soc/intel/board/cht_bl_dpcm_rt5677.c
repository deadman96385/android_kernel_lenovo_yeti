...
static struct snd_soc_dai_link cht_dailink[] = {
	...
	[CHT_DPCM_AUDIO] = {
		...
		.codec_dai_name = "snd-soc-dummy-dai",    // set codec and codecDAI name to dummy, because BE is dynamic and will change depending on runtime config
		.platform_name = "sst-platform",
		.init = cht_audio_init,
		.ignore_suspend = 1,    // set this flag allows BE work in a hostless mode where the CPU is not transferring data like a BT phone call
		.dynamic = 1,
		.ops = &cht_aif1_ops,
		.dpcm_playback = 1,
	},
	...
	/* Back ends */
	{
		...
		.codec_dai_name = "rt5677-aif1",
		.codec_name = "rt5677.0-002c",
		.be_hw_params_fixup = cht_codec_fixup,    // fixup function is used by machine driver to (re)configure DAI based on the FE hw params
		.ignore_suspend = 1,
		.ops = &cht_be_ssp2_ops,
	},
	{
		...
		.platform_name = "snd-soc-dummy",       // a BE DAI link can set cpu to a dummy device if the code is a device that is managed externally
		.no_pcm = 1,
		.codec_name = "snd-soc-dummy",    // likewise a BE DAI can also set a dummy codec DAI if the codec DAI is managed is managed by firmware
		.codec_dai_name = "snd-soc-dummy-dai",
		.ignore_suspend = 1,
	},
	{
